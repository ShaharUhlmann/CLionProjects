...
class ExInfo:
    ...
    def get_presubmit_main_src(self):
        lang_ext = 'c' if self.get_lang() == "C" else 'cpp'
        return f"{self.get_presub_src_dir()}/presubmit.{lang_ext}"

    def get_additional_srcs(self):
        # return []
        ...
        return [f"{self.ex_base_dir}/autotest_src/{i}" for i in self.autotests_additional_srcs]

    def get_wrap_symbols(self):
        if self.is_presubmit:
            return self.presubmit_compilation_wrap_symbols
        return self.autotests_compilation_wrap_symbols
...
def get_ex_info_from_folder(ex_folder: str, is_presubmit: bool = True) -> ExInfo:
